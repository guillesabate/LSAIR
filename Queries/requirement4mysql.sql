...
FROM person AS p
    INNER JOIN employee AS emp ON p.personID = emp.employeeID
    INNER JOIN luggagehandler AS lh ON emp.employeeID = lh.luggagehandlerID
    INNER JOIN flightluggagehandler AS flh ON lh.luggagehandlerID = flh.luggageHandlerID
WHERE flh.flightID NOT IN (
    SELECT DISTINCT flha.flightID
...
SELECT p.personID, p.name, p.surname, emp.salary,
        ...
        WHERE e.employeeID = l.luggagehandlerID) AS 'average salary',
        COUNT(DISTINCT flh.luggageHandlerID) AS 'luggage handlers'
FROM person AS p
    INNER JOIN employee AS emp ON p.personID = emp.employeeID,
    luggagehandler AS lh
    INNER JOIN flightluggagehandler AS flh ON lh.luggagehandlerID = flh.luggageHandlerID
WHERE emp.employeeID = lh.luggagehandlerID AND flh.flightID NOT IN (
    SELECT DISTINCT flha.flightID
    FROM flightluggagehandler AS flha
    WHERE flha.luggageHandlerID != flh.luggageHandlerID)
GROUP BY (p.personID)
...
SELECT p.name, p.email, c.name, l.color, l.brand,
    (SELECT (lug.weight)
    FROM luggage AS lug
    WHERE lug.passengerID = l.passengerID  order by lug.weight limit 1) AS weight,
    hl.size_x*hl.size_y*hl.size_z AS volume, cl.extra_cost, so.fragile
FROM country c
